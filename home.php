...
while ($getS_row = $getS->fetch(PDO::FETCH_ASSOC)) {
    ...
    $getuser_sql = "SELECT Username,Userphoto,Fullname FROM ncm_accounts WHERE id=:getuserid";
...
<!--========================[ Center col2 ]===============================-->
        <div align="left">
            ...
            <?php
            $uid = $_SESSION['usid'];
            $sqlQ = "SELECT aSetup FROM ncm_accounts WHERE id=:uid";
            ...
            if ($aSetupProg == 100 ) {
                $uid = $_SESSION['usid'];
                $sqlQ = "UPDATE ncm_accounts SET aSetup = :aSetupProg WHERE id = :uid";
                $sqlQ_check = $conn->prepare($sqlQ);
        ...
        <div align="left">
         <div id="centerCol3" style="width: 310px">
          ...
          <div id="trendingPostsPagesLoading" style="display: none;background: url(imgs/loading_video.gif) center center no-repeat;width: 100%;height: 100px">
            <span style="display: none; color: #03A9F4; font-size: 16px; margin: 10 15;text-align: center; position: relative; top: 60px; " class="loadingm">The loading takes longer than usual.</span>
          </div>
          <div id="trendingPostsPages">
            ...
            function trendingPostsPages() {
                $.ajax({
                    ...
                    setTimeout(function () {
                      $('#trendingPostsPagesLoading .loadingm').slideDown(1);
                    }, 5000);
                    },
